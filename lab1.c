...
int WriteToFile(int in, int out, char* filepath, struct stat statbuf)
{
    ...
    write(out, &is_catalog, sizeof(int));
    ...
    int a = lseek(out, -filesize - sizeof(int), SEEK_CUR); 
    //записываем сколько надо считать                     
...
int ReadCatalog(int in)
{
    ...
    if (IsCatalog != 0)
    {
        ...
        //и заходим в нее
        mkdir(outfile,//чтобы не писать sudo права доступа на всех
